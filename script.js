...
const ROOM_PLANTS = {
  bedroom: [
    ...
    { name:"Rubber Plant",  emoji:"🌳", benefit:"Large leaves absorb significant amounts of airborne toxins from new furniture and paint. Great statement plant.", care:"Easy", tag:"🛋️ VOC Absorber" },
  ],
  kitchen: [
  ...
  living: [
    ...
    { name:"Rubber Plant",  emoji:"🌳", benefit:"Large leaves trap dust and absorb VOCs from sofas and carpets. A beautiful statement plant for living rooms.", care:"Easy", tag:"🛋️ Dust Trap" },
    { name:"Bamboo Palm",   emoji:"🌴", benefit:"One of the most effective filters for benzene and trichloroethylene. Tall and elegant — suits large living rooms.", care:"Moderate", tag:"🌬️ Deep Purifier" },
  ...
  office: [
    { name:"Snake Plant",   emoji:"🌵", benefit:"Survives low light and infrequent watering — perfect for a busy desk. Filters toxins from electronics and printers.", care:"Very Easy", tag:"🖥️ Desk Friendly" },
    ...
    { name:"ZZ Plant",      emoji:"🌿", benefit:"Virtually indestructible — survives low light and weeks without water. Removes xylene from printer ink and toner.", care:"Very Easy", tag:"🏋️ Tough as Nails" },
    { name:"Spider Plant",  emoji:"🌱", benefit:"Removes chemical fumes from printers and electronics. Great on high shelves where it can trail down naturally.", care:"Easy", tag:"🖨️ Printer Fumes" },
    { name:"Bamboo Palm",   emoji:"🌴", benefit:"Filters pollutants from electronics and acts as a natural humidifier against dry office air from AC systems.", care:"Moderate", tag:"❄️ AC Dry Air" },
...
function updateProfileUI(data) {
  ...
  if (guest && user) {
    ...
    document.getElementById('profileHeroAvatar').textContent = data.avatar || '🌿';
    document.getElementById('profileHeroName').textContent = data.name;
    const metaParts = [];
    if (data.city) metaParts.push('📍 ' + data.city);
    if (data.home) metaParts.push('🏠 ' + data.home);
    document.getElementById('profileHeroMeta').textContent = metaParts.join('   ');
  }
...
function loadProfile() {
  ...
  if (data.avatar) {
    ...
    document.querySelectorAll('.avatar-opt').forEach(b => b.classList.toggle('active', b.dataset.avatar === data.avatar));
  }
...
document.getElementById('saveProfile')?.addEventListener('click', () => {
  const data = {
    ...
    avatar:    document.getElementById('profileAvatar').textContent
  };
  localStorage.setItem('airleaf_profile', JSON.stringify(data));
  updateProfileUI(data);
  // 🔥 Track profile saved event
...
function renderProfileSummary() {
  ...
  const trend = scores.length >= 2 ? (scores[0] > scores[1] ? '📈 Improving' : scores[0] < scores[1] ? '📉 Declining' : '➡️ Stable') : '—';
  el.innerHTML = `
...
document.getElementById('avatarPicker')?.addEventListener('click', e => {
  ...
  document.querySelectorAll('.avatar-opt').forEach(b => b.classList.toggle('active', b === btn));
